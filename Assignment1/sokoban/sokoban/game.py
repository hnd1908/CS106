...
class Game:
    def __init__(self, window):
        ...
        # đoạn này em code thêm để chụp ảnh trạng thái kết thúc
        # self.all_targets_filled = False
    ...
    # đoạn này em code thêm, thay thế cho đoạn code trên để chụp ảnh trạng thái kết thúc
    # def process_event(self, event):
    #     if event.type == QUIT:
    #         pygame.quit()
    #         sys.exit()
    ...
    #             self.player.move(event.key, self.level, self.player_interface)
    #             if self.has_win():
    #                 self.all_targets_filled = True
    #                 self.update_screen()  # Vẽ lại màn hình trước khi kiểm tra điều kiện dừng lại
    #         if event.key == K_RETURN and self.has_win() and self.all_targets_filled:
    #             # Tăng level
    #             self.index_level += 1
    #             if self.index_level == 17:
    #                 self.index_level = 1
    ...
    #             if self.level.structure[y][x] == SOKOBAN.TARGET:
    #                 nb_missing_target += 1
    #             elif self.level.structure[y][x] == SOKOBAN.BOX and self.level.structure[y][x] != SOKOBAN.TARGET_FILLED:
    #                 return False
